...
<pre><code>Hi {{firstName}}, your check is {{total}} euros.</p>
</code></pre><p>Some special values (like <code>{{firstName}}</code>, <code>{{lastName}}</code>, <code>{{chatId}}</code>, etc) are always available.</p>
<p>It&#39;s possible to specify more content versions for the same message, one will be randomly chosen with an even distribution.</p>
